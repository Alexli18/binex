...
def _resolve_workflow_path(run) -> Path | None:
...
def _load_workflow_node_specs(run) -> dict[str, dict]:
...
def _index_artifacts(artifacts) -> tuple[dict[str, list[dict]], dict[str, dict]]:
...
def _apply_filters(
    nodes: list[dict], node_filter: str | None, errors_only: bool,
) -> list[dict]:
